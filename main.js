...
;// ./src/utils/date-and-time/currentTime.js
/**
...
;// ./src/img/backgrounds/01.jpg
const _01_namespaceObject = __webpack_require__.p + "20281eaf7f3981226ac7.jpg";
;// ./src/img/backgrounds/02.jpg
const _02_namespaceObject = __webpack_require__.p + "508d0e5e4d7b6dad287b.jpg";
;// ./src/img/backgrounds/03.jpg
const _03_namespaceObject = __webpack_require__.p + "9bf9f9292a6b99df332a.jpg";
;// ./src/img/backgrounds/04.jpg
const _04_namespaceObject = __webpack_require__.p + "721bd82379d81ac683dc.jpg";
;// ./src/components/dynamic-wallpapers/DynamicWallpapers.js
...
const DynamicWallpapers_images = {
  image1: _01_namespaceObject,
  image2: _02_namespaceObject,
  image3: _03_namespaceObject,
  image4: _04_namespaceObject
};

/**
 * Класс для работы с динамической сменой заставки
 */
class DynamicWallpapers {
  constructor() {
  ...
  changeWallpapers() {
    ...
    if (time >= "00:00" && time < "06:00") {
      this.body.style.backgroundImage = `url(${DynamicWallpapers_images.image1})`;
    } else if (time >= "06:00" && time < "12:00") {
      this.body.style.backgroundImage = `url(${DynamicWallpapers_images.image2})`;
    } else if (time >= "12:00" && time < "18:00") {
      this.body.style.backgroundImage = `url(${DynamicWallpapers_images.image3})`;
    } else if (time >= "18:00" && time <= "23:59") {
      this.body.style.backgroundImage = `url(${DynamicWallpapers_images.image4})`;
    }
  }
}
;// ./src/utils/date-and-time/currentDate.js
/**
...
;// ./src/components/time-date/date-display/DateDisplay.js
...
;// ./src/components/time-date/time-dispaly/TimeDisplay.js
...
;// ./src/utils/city-and-weather/getCurrentCity.js
...
async function getCurrentCity() {
  try {
    ...
    const response = await lib_axios.get(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${coordinates.latitude}&lon=${coordinates.longitude}&zoom=18&addressdetails=1`);
    const {
      city
...
;// ./src/components/weather/city/City.js
...
;// ./src/utils/city-and-weather/getInfoWeather.js
 ...
 */
async function getInfoWeather(weather) {
  try {
    const coordinates = JSON.parse(localStorage.getItem("coordinates"));

    // Если координаты отсутствуют, возвращаем null
    if (!coordinates) return null;
    const response = await lib_axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${coordinates.latitude}&longitude=${coordinates.longitude}&current=temperature_2m,wind_speed_10m&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m&current=weather_code`);
    if (weather === "weather") {
...
;// ./src/data/codes-weather.json
const codes_weather_namespaceObject = /*#__PURE__*/JSON.parse('[{"дождь":[61,63,65]},{"ясное небо":[0]},{"снег":[71,73,75]},{"пасмурно":[1,2,3]}]');
;// ./src/components/weather/current-weather/CurrentWeather.js
...
class CurrentWeather {
  ...
  async getCurrentWeather() {
    try {
      ...
      codes_weather_namespaceObject.forEach(currWeather => {
...
;// ./src/components/weather/temperature/Temperature.js
...
class WeatherWidget {
  constructor(city, temperature, weather) {
    ...
    if (city instanceof Object && temperature instanceof Object && weather instanceof Object) {
      this.city = city;
  ...
  async renderWeatherWidget() {
    try {
      ...
      this.temperatureElem.textContent = await this.temperature.getTemperature();
...
;// ./src/components/tasks/storage-handler/StorageHandler.js
/**
...
;// ./src/components/tasks/tasks-render/TasksRender.js
 ...
 */
class TasksRender extends StorageHandler {
  constructor() {
    super();
...
;// ./src/components/tasks/task-form/TaskForm.js
 ...
 */
class TaskForm extends StorageHandler {
   ...
   */
  addTask() {
    const taskInputValue = this.taskFormTag.querySelector(".task-input").value.trim();
...
;// ./src/components/tasks/task-actions/TaskActions.js
 ...
 */
class TaskActions extends StorageHandler {
  constructor(element) {
    super();
  ...
  addFlagCheckbox(e) {
    ...
    const taskCheckbox = target.closest(".task-elem")?.querySelector(".task-checkbox");
...
;// ./src/utils/geolocation-coordinates/getGeolocationCoordinates.js
/**
 * @async
 ...
 */
async function getGeolocationCoordinates() {
...
;// ./src/utils/geolocation-coordinates/saveGeolocationCoordinates.js
 ...
 */
async function saveGeolocationCoordinates() {
  if (navigator.geolocation) {
    try {
